...
router.post("/questions", adminProtect, async (req, res) => {
  try {
    const { question, options, correctAnswer, examType, category } = req.body;
...
router.get("/questions", protect, async (req, res) => {
  try {
    ...
    const databaseQuestions = await Question.find({ examType });
...
router.post("/submit", protect, async (req, res) => {
  try {
    ...
    const accuracyPercentage = fullQuestionPool.length > 0 ? parseFloat(((calculatedScore / fullQuestionPool.length) * 100).toFixed(2)) : 0;
...
router.get("/leaderboard", protect, async (req, res) => {
  try {
    ...
    const leaderboard = Object.values(bestScores).sort((a, b) => b.score - a.score).slice(0, 100);
...
router.delete("/questions/delete-all", adminProtect, async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: examType ? `Successfully deleted ${result.deletedCount} ${examType} questions.` : `Successfully deleted ${result.deletedCount} questions.`,
...
router.put("/admin/questions/:id", adminProtect, async (req, res) => {
  try {
    ...
    const updatedQuestion = await Question.findByIdAndUpdate(
      req.params.id,
      { question, options, correctAnswer, examType: examType || "NTPC", category: category || "General Awareness" },
      { new: true }
    );
...
router.get("/admin/results", adminProtect, async (req, res) => {
  try {
    const examType = req.query.exam;
    const filter = examType ? { examType } : {};
    const results = await Result.find(filter).populate("userId", "username email").sort({ createdAt: -1 });
    res.status(200).json(results);
  } catch (err) {
